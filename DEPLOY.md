...
## 7. Как обновляются данные на сервере
...
## 8. Админ-доступ (ограничить правки пользователей)

В `.env` на сервере (или перед `docker run`):
...
Пересоздайте контейнер (`bash fixdb.sh` или `BUILD=1 bash deploy.sh`).

- Без пароля: сайт в режиме разработки (правки открыты).
- С паролем: гости только смотрят; кнопка **Вход** → админ добавляет узлы и провода.
